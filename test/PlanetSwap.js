...
const planetPair = "0x89fD5DcF5D7C9FEbA8c8106A5Abc09325fC2a39e"
const ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";

describe("PlanetSwap", function () {
  ...
  async function deploySwapFixture() {
    ...
    const Swap = await ethers.getContractFactory("PlanetSwap");
  ...
  describe("Deployment", function () {
    it("deploy and update proxy", async function () {
      ...
      expect(await swap.addPair(planetPair, tokens.WBNB.address, tokens.BUSD.address)).not.to.be.reverted;
      const pairUpdate = await swapUpdate.pairs(0);
      expect(pairUpdate.pairAddress).to.equal(planetPair);
      ...
      expect(pair.pairAddress).to.equal(planetPair);
  ...
  describe("Functions", function () {
    describe("addPair", function () {
      it("only DEFAULT_ADMIN_ROLE", async function () {
        ...
        await expect(swap.addPair(planetPair, tokens.WBNB.address, tokens.BUSD.address)).not.to.be.reverted;
      ...
      it("token is not address(0)", async function () {
        ...
        expect(swap.addPair(planetPair, ZERO_ADDRESS, tokens.BUSD.address)).to.be.revertedWith("INVALID_ADDRESS(0)");
        expect(swap.addPair(planetPair, tokens.WBNB.address, ZERO_ADDRESS)).to.be.revertedWith("INVALID_ADDRESS(0)");
      });

      it("token cannot be repeated", async function () {
        const { swap } = await loadFixture(deploySwapFixture);
        await expect(swap.addPair(planetPair, tokens.WBNB.address, tokens.BUSD.address)).not.to.be.reverted;
        await expect(swap.addPair(planetPair, tokens.WBNB.address, tokens.BUSD.address)).to.be.revertedWithCustomError(swap, "PairExsits");
      });
    });
    ...
    describe("calculateAmount", function () {
      it("the token1 cost of getting 1 token0", async function () {
        ...
        const token1Cost = await swap.calculateAmount(planetPair, ethers.parseEther("1"), 0, true, 25, 10000);
      ...
      it("spending 1 token0 to get token1", async function () {
        ...
        const token1Get = await swap.calculateAmount(planetPair, ethers.parseEther("1"), 0, false, 25, 10000);
      ...
      it("the token0 cost of getting 1 token1", async function () {
        ...
        const token0Cost = await swap.calculateAmount(planetPair, ethers.parseEther("1"), 1, true, 25, 10000);
      ...
      it("spending 1 token1 to get token0", async function () {
        ...
        const token0Get = await swap.calculateAmount(planetPair, ethers.parseEther("1"), 1, false, 25, 10000);
      ...
      it("tokenNum can only be 0 or 1", async function () {
        ...
        expect(swap.calculateAmount(planetPair, ethers.parseEther("1"), 3, false)).to.be.revertedWith("only 0 or 1");
      });
    ...
    describe("swap", function () {
      ...
      it("swap WBNB to get BUSD", async function () {
        const token0Cost = await swap.calculateAmount(planetPair, ethers.parseEther("1"), 1, true, 25, 10000);
        console.log("the token0 cost of getting 1 token1: %s", ethers.formatEther(token0Cost));
        expect(await swap.swap(planetPair, ethers.parseEther("1"), tokens.BUSD.address, token0Cost, 25, 10000)).not.to.be.reverted;
        await showERC20Balance(tokens.WBNB.address, swap.target);
        await showERC20Balance(tokens.BUSD.address, swap.target);
      });
      ...
      it("swap BUSD to get WBNB", async function () {
        ...
        const token0Get = await swap.calculateAmount(planetPair, ethers.parseEther("1"), 1, false, 25, 10000);
        console.log("spending 1 token1 to get token0: %s", ethers.formatEther(token0Get));
        expect(await swap.swap(planetPair, token0Get, tokens.WBNB.address, ethers.parseEther("1"), 25, 10000)).not.to.be.reverted;
        await showERC20Balance(tokens.WBNB.address, swap.target);
        await showERC20Balance(tokens.BUSD.address, swap.target);
      });
    });
