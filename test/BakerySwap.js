...
const BakerySwapPair = "0x50AA03568bdc5323A38a0Dc60017E29400347A9C"
const ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";

describe("BakerySwap", function () {
  ...
  async function deploySwapFixture() {
    ...
    const Swap = await ethers.getContractFactory("BakerySwap");
  ...
  describe("Deployment", function () {
    it("deploy and update proxy", async function () {
      ...
      expect(await swap.addPair(BakerySwapPair, tokens.WBNB.address, tokens.ibBNB.address)).not.to.be.reverted;
      const pairUpdate = await swapUpdate.pairs(0);
      expect(pairUpdate.pairAddress).to.equal(BakerySwapPair);
      expect(pairUpdate.token0).to.equal(tokens.WBNB.address);
      expect(pairUpdate.token1).to.equal(tokens.ibBNB.address);
      const pair = await swap.pairs(0);
      expect(pair.pairAddress).to.equal(BakerySwapPair);
      expect(pair.token0).to.equal(tokens.WBNB.address);
      expect(pair.token1).to.equal(tokens.ibBNB.address);
    });
  });
  ...
  describe("Functions", function () {
    describe("addPair", function () {
      it("only DEFAULT_ADMIN_ROLE", async function () {
        ...
        await expect(swap.addPair(BakerySwapPair, tokens.WBNB.address, tokens.ibBNB.address)).not.to.be.reverted;
        await expect(swap.connect(keeper1).addPair("0xFc7AC742C051Ca19B9eA5725280eAAB5b3073534", tokens.USDC.address, tokens.ibBNB.address)).to.be.reverted;
      });

      it("token is not address(0)", async function () {
        const { swap } = await loadFixture(deploySwapFixture);
        expect(swap.addPair(ZERO_ADDRESS, tokens.WBNB.address, tokens.ibBNB.address)).to.be.revertedWith("INVALID_ADDRESS(0)");
        expect(swap.addPair(BakerySwapPair, ZERO_ADDRESS, tokens.ibBNB.address)).to.be.revertedWith("INVALID_ADDRESS(0)");
        expect(swap.addPair(BakerySwapPair, tokens.WBNB.address, ZERO_ADDRESS)).to.be.revertedWith("INVALID_ADDRESS(0)");
      });

      it("token cannot be repeated", async function () {
        const { swap } = await loadFixture(deploySwapFixture);
        await expect(swap.addPair(BakerySwapPair, tokens.WBNB.address, tokens.ibBNB.address)).not.to.be.reverted;
        await expect(swap.addPair(BakerySwapPair, tokens.WBNB.address, tokens.ibBNB.address)).to.be.revertedWithCustomError(swap, "PairExsits");
      });
    });
    ...
    describe("calculateAmount", function () {
      it("the token1 cost of getting 1 token0", async function () {
        ...
        const token1Cost = await swap.calculateAmount(BakerySwapPair, ethers.parseEther("0.01"), 0, true, 3, 1000);
      ...
      it("spending 1 token0 to get token1", async function () {
        ...
        const token1Get = await swap.calculateAmount(BakerySwapPair, ethers.parseEther("1"), 0, false, 3, 1000);
      ...
      it("the token0 cost of getting 1 token1", async function () {
        ...
        const token0Cost = await swap.calculateAmount(BakerySwapPair, ethers.parseEther("0.01"), 1, true, 3, 1000);
      ...
      it("spending 1 token1 to get token0", async function () {
        ...
        const token0Get = await swap.calculateAmount(BakerySwapPair, ethers.parseEther("1"), 1, false, 3, 1000);
      ...
      it("tokenNum can only be 0 or 1", async function () {
        ...
        expect(swap.calculateAmount(BakerySwapPair, ethers.parseEther("1"), 3, false)).to.be.revertedWith("only 0 or 1");
      });

      it("getting token0 > reserve0", async function () {
        const { swap } = await loadFixture(deploySwapFixture);

        expect(swap.calculateAmount(BakerySwapPair, ethers.parseEther("1"), 0, true, 3, 1000)).to.be.revertedWith("amountGet >= reserve");
      });

      it("getting token1 > reserve1", async function () {
        const { swap } = await loadFixture(deploySwapFixture);

        expect(swap.calculateAmount(BakerySwapPair, ethers.parseEther("1"), 1, true, 3, 1000)).to.be.revertedWith("amountGet >= reserve");
      });
    ...
    describe("swap", function () {
      ...
      beforeEach(async function () {
        ...
        await showERC20Balance(tokens.ibBNB.address, swap.target);
        await expect(swap.addPair(BakerySwapPair, tokens.WBNB.address, tokens.ibBNB.address)).not.to.be.reverted;
    })

      it("swap WBNB to get ibBNB", async function () {
        const token0Cost = await swap.calculateAmount(BakerySwapPair, ethers.parseEther("0.01"), 1, true, 3, 1000);
        console.log("the token0 cost of getting 0.01 token1: %s", ethers.formatEther(token0Cost));
        expect(await swap.swap(BakerySwapPair, ethers.parseEther("0.01"), tokens.ibBNB.address, token0Cost, 3, 1000)).not.to.be.reverted;
        await showERC20Balance(tokens.WBNB.address, swap.target);
        await showERC20Balance(tokens.ibBNB.address, swap.target);
      });

      it("swap ibBNB to get WBNB", async function () {
        const token0Cost = await swap.calculateAmount(BakerySwapPair, ethers.parseEther("0.01"), 1, true, 3, 1000);
        expect(await swap.swap(BakerySwapPair, ethers.parseEther("0.01"), tokens.ibBNB.address, token0Cost, 3, 1000)).not.to.be.reverted;
        await showERC20Balance(tokens.WBNB.address, swap.target);
        await showERC20Balance(tokens.ibBNB.address, swap.target);

        const token0Get = await swap.calculateAmount(BakerySwapPair, ethers.parseEther("0.01"), 1, false, 3, 1000);
        console.log("spending 0.01 token1 to get token0: %s", ethers.formatEther(token0Get));
        expect(await swap.swap(BakerySwapPair, token0Get, tokens.WBNB.address, ethers.parseEther("0.01"), 3, 1000)).not.to.be.reverted;
        await showERC20Balance(tokens.WBNB.address, swap.target);
        await showERC20Balance(tokens.ibBNB.address, swap.target);
      });
    });
