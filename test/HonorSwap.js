...
const honorPair = "0xB606bD4Ee10db0A5d115c1d84b5b0A97D2f9101B"
const ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";

describe("HonorSwap", function () {
  ...
  async function deploySwapFixture() {
    ...
    const Swap = await ethers.getContractFactory("HonorSwap");
  ...
  describe("Deployment", function () {
    it("deploy and update proxy", async function () {
      ...
      expect(await swap.addPair(honorPair, tokens.ALPACA.address, tokens.WBNB.address)).not.to.be.reverted;
      const pairUpdate = await swapUpdate.pairs(0);
      expect(pairUpdate.pairAddress).to.equal(honorPair);
      expect(pairUpdate.token0).to.equal(tokens.ALPACA.address);
      expect(pairUpdate.token1).to.equal(tokens.WBNB.address);
      const pair = await swap.pairs(0);
      expect(pair.pairAddress).to.equal(honorPair);
      expect(pair.token0).to.equal(tokens.ALPACA.address);
  ...
  describe("Functions", function () {
    describe("addPair", function () {
      it("only DEFAULT_ADMIN_ROLE", async function () {
        ...
        await expect(swap.addPair(honorPair, tokens.ALPACA.address, tokens.WBNB.address)).not.to.be.reverted;
      ...
      it("token is not address(0)", async function () {
        const { swap } = await loadFixture(deploySwapFixture);
        expect(swap.addPair(ZERO_ADDRESS, tokens.ALPACA.address, tokens.WBNB.address)).to.be.revertedWith("INVALID_ADDRESS(0)");
        expect(swap.addPair(honorPair, ZERO_ADDRESS, tokens.WBNB.address)).to.be.revertedWith("INVALID_ADDRESS(0)");
        expect(swap.addPair(honorPair, tokens.ALPACA.address, ZERO_ADDRESS)).to.be.revertedWith("INVALID_ADDRESS(0)");
      });

      it("token cannot be repeated", async function () {
        const { swap } = await loadFixture(deploySwapFixture);
        await expect(swap.addPair(honorPair, tokens.ALPACA.address, tokens.WBNB.address)).not.to.be.reverted;
        await expect(swap.addPair(honorPair, tokens.ALPACA.address, tokens.WBNB.address)).to.be.revertedWithCustomError(swap, "PairExsits");
      });
    });
    ...
    describe("calculateAmount", function () {
      it("the token1 cost of getting 1 token0", async function () {
        ...
        const token1Cost = await swap.calculateAmount(honorPair, ethers.parseEther("1"), 0, true, 25, 10000);
      ...
      it("spending 1 token0 to get token1", async function () {
        ...
        const token1Get = await swap.calculateAmount(honorPair, ethers.parseEther("1"), 0, false, 25, 10000);
      ...
      it("the token0 cost of getting 0.1 token1", async function () {
        ...
        const token0Cost = await swap.calculateAmount(honorPair, ethers.parseEther("0.1"), 1, true, 25, 10000);
      ...
      it("spending 1 token1 to get token0", async function () {
        ...
        const token0Get = await swap.calculateAmount(honorPair, ethers.parseEther("1"), 1, false, 25, 10000);
      ...
      it("tokenNum can only be 0 or 1", async function () {
        ...
        expect(swap.calculateAmount(honorPair, ethers.parseEther("1"), 3, false)).to.be.revertedWith("only 0 or 1");
      });

      it("getting token1 > reserve1", async function () {
        const { swap } = await loadFixture(deploySwapFixture);

        expect(swap.calculateAmount(honorPair, ethers.parseEther("1"), 1, true, 25, 10000)).to.be.revertedWith("amountGet >= reserve");
      });
    ...
    describe("swap", function () {
      ...
      beforeEach(async function () {
        ...
        await showERC20Balance(tokens.ALPACA.address, swap.target);
        await showERC20Balance(tokens.WBNB.address, swap.target);
        await expect(swap.addPair(honorPair, tokens.ALPACA.address, tokens.WBNB.address)).not.to.be.reverted;
      })

      it("swap WBNB to get ALPACA", async function () {
        const token0Get = await swap.calculateAmount(honorPair, ethers.parseEther("1"), 1, false, 25, 10000);
        console.log("spending 1 token1 to get token0: %s", ethers.formatEther(token0Get));
        expect(await swap.swap(honorPair, token0Get, tokens.ALPACA.address, ethers.parseEther("1"), 25, 10000)).not.to.be.reverted;
        await showERC20Balance(tokens.ALPACA.address, swap.target);
        await showERC20Balance(tokens.WBNB.address, swap.target);
      });

      it("swap ALPACA to get WBNB", async function () {
        const token0Get = await swap.calculateAmount(honorPair, ethers.parseEther("1"), 1, false, 25, 10000);
        expect(await swap.swap(honorPair, token0Get, tokens.ALPACA.address, ethers.parseEther("1"), 25, 10000)).not.to.be.reverted;
        await showERC20Balance(tokens.ALPACA.address, swap.target);
        await showERC20Balance(tokens.WBNB.address, swap.target);
        ...
        const token1Get = await swap.calculateAmount(honorPair, ethers.parseEther("1"), 0, false, 25, 10000);
        console.log("spending 1 token0 to get token1: %s", ethers.formatEther(token1Get));
        expect(await swap.swap(honorPair, token1Get, tokens.WBNB.address, ethers.parseEther("1"), 25, 10000)).not.to.be.reverted;
        await showERC20Balance(tokens.ALPACA.address, swap.target);
        await showERC20Balance(tokens.WBNB.address, swap.target);
      });
    });
