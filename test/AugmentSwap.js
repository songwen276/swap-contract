...
const augmentPair = "0x3994996ecff44498E0F55c71D123D47b45BA3E93"
const ZERO_ADDRESS = "0x0000000000000000000000000000000000000000";

describe("AugmentSwap", function () {
  ...
  async function deploySwapFixture() {
    ...
    const Swap = await ethers.getContractFactory("AugmentSwap");
  ...
  describe("Deployment", function () {
    it("deploy and update proxy", async function () {
      ...
      expect(await swap.addPair(augmentPair, tokens.USC.address, tokens.USDT.address)).not.to.be.reverted;
      const pairUpdate = await swapUpdate.pairs(0);
      expect(pairUpdate.pairAddress).to.equal(augmentPair);
      expect(pairUpdate.token0).to.equal(tokens.USC.address);
      expect(pairUpdate.token1).to.equal(tokens.USDT.address);
      const pair = await swap.pairs(0);
      expect(pair.pairAddress).to.equal(augmentPair);
      expect(pair.token0).to.equal(tokens.USC.address);
      expect(pair.token1).to.equal(tokens.USDT.address);
    });
  });
  ...
  describe("Functions", function () {
    describe("addPair", function () {
      it("only DEFAULT_ADMIN_ROLE", async function () {
        ...
        await expect(swap.addPair(augmentPair, tokens.USC.address, tokens.USDT.address)).not.to.be.reverted;
        await expect(swap.connect(keeper1).addPair("0xFc7AC742C051Ca19B9eA5725280eAAB5b3073534", tokens.USDC.address, tokens.USDT.address)).to.be.reverted;
      });

      it("token is not address(0)", async function () {
        const { swap } = await loadFixture(deploySwapFixture);
        expect(swap.addPair(ZERO_ADDRESS, tokens.USC.address, tokens.USDT.address)).to.be.revertedWith("INVALID_ADDRESS(0)");
        expect(swap.addPair(augmentPair, ZERO_ADDRESS, tokens.USDT.address)).to.be.revertedWith("INVALID_ADDRESS(0)");
        expect(swap.addPair(augmentPair, tokens.USC.address, ZERO_ADDRESS)).to.be.revertedWith("INVALID_ADDRESS(0)");
      });

      it("token cannot be repeated", async function () {
        const { swap } = await loadFixture(deploySwapFixture);
        await expect(swap.addPair(augmentPair, tokens.USC.address, tokens.USDT.address)).not.to.be.reverted;
        await expect(swap.addPair(augmentPair, tokens.USC.address, tokens.USDT.address)).to.be.revertedWithCustomError(swap, "PairExsits");
      });
    });
    ...
    describe("calculateAmount", function () {
      it("the token1 cost of getting 1 token0", async function () {
        ...
        const token1Cost = await swap.calculateAmount(augmentPair, ethers.parseEther("1"), 0, true, 25, 10000);
      ...
      it("spending 1 token0 to get token1", async function () {
        ...
        const token1Get = await swap.calculateAmount(augmentPair, ethers.parseEther("1"), 0, false, 25, 10000);
      ...
      it("the token0 cost of getting 1 token1", async function () {
        ...
        const token0Cost = await swap.calculateAmount(augmentPair, ethers.parseEther("1"), 1, true, 25, 10000);
      ...
      it("spending 1 token1 to get token0", async function () {
        ...
        const token0Get = await swap.calculateAmount(augmentPair, ethers.parseEther("1"), 1, false, 25, 10000);
      ...
      it("tokenNum can only be 0 or 1", async function () {
        ...
        expect(swap.calculateAmount(augmentPair, ethers.parseEther("1"), 3, false)).to.be.revertedWith("only 0 or 1");
      });
    ...
    describe("swap", function () {
      ...
      beforeEach(async function () {
        ...
        // await showERC20Balance(tokens.USC.address, deployer.address);
        ...
        await showERC20Balance(tokens.USC.address, swap.target);
        await showERC20Balance(tokens.USDT.address, swap.target);
        await expect(swap.addPair(augmentPair, tokens.USC.address, tokens.USDT.address)).not.to.be.reverted;
      });


      it("swap USC to get USDT", async function () {
        const token1Cost = await swap.calculateAmount(augmentPair, ethers.parseEther("5"), 0, true, 25, 10000);
        console.log("the token1 cost of getting 5 token0: %s", ethers.formatEther(token1Cost));
        expect(await swap.swap(augmentPair, ethers.parseEther("5"), tokens.USC.address, token1Cost, 25, 10000)).not.to.be.reverted;
        await showERC20Balance(tokens.USC.address, swap.target);
        await showERC20Balance(tokens.USDT.address, swap.target);

        const token0Cost = await swap.calculateAmount(augmentPair, ethers.parseEther("1"), 1, true, 25, 10000);
        console.log("the token0 cost of getting 1 token1: %s", ethers.formatEther(token0Cost));
        expect(await swap.swap(augmentPair, ethers.parseEther("1"), tokens.USDT.address, token0Cost, 25, 10000)).not.to.be.reverted;
        await showERC20Balance(tokens.USC.address, swap.target);
        await showERC20Balance(tokens.USDT.address, swap.target);
      });

      it("swap USDT to get USC", async function () {
        const token0Get = await swap.calculateAmount(augmentPair, ethers.parseEther("1"), 1, false, 25, 10000);
        console.log("spending 1 token1 to get token0: %s", ethers.formatEther(token0Get));
        expect(await swap.swap(augmentPair, token0Get, tokens.USC.address, ethers.parseEther("1"), 25, 10000)).not.to.be.reverted;
        await showERC20Balance(tokens.USC.address, swap.target);
        await showERC20Balance(tokens.USDT.address, swap.target);
      });
