...
import "./interface/INomiswapCallee.sol";
import "./interface/INomiswapStablePair.sol";

contract NomiswapStableSwap is
    Initializable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function swap(
        ...
        CallData memory data = CallData(pair, amountCostMax);
        if (tokenGet == pair.token0) {
            INomiswapStablePair(pairAddress).swap(
                amountGet,
                0,
                address(this),
                abi.encode(data)
    ...
    function nomiswapCall(
        ...
        if (amount0 > 0) {
            ...
            uint amount1Cost = INomiswapStablePair(callData.pair.pairAddress)
                .getAmountIn(callData.pair.token1, amount0);
            require(
            ...
            uint amount0Cost = INomiswapStablePair(callData.pair.pairAddress)
                .getAmountIn(callData.pair.token0, amount1);
            require(
    ...
    function calculateAmount(
        ...
        if (tokenNum == 0 && isGet) {
            ...
            calAmount = INomiswapStablePair(pairAddress).getAmountIn(
                pair.token1,
                amount
            );
        } else if (tokenNum == 0 && !isGet) {
            // 计算花费token0能够获取多少token1
            calAmount = INomiswapStablePair(pairAddress).getAmountOut(
                pair.token0,
                amount
            );
