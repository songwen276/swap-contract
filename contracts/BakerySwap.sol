...
import "./interface/IBakerySwapPair.sol";
import "./utils/Calculate.sol";

contract BakerySwap is Initializable, AccessControlUpgradeable {
    error PairExsits();
    ...
    function swap(
        ...
        bytes memory bdata = abi.encode(data);
        if (tokenGet == pair.token0) {
            BakerySwapCall(amountGet, 0, bdata);
            IBakerySwapPair(pairAddress).swap(
                amountGet,
                0,
                address(this)
            );
        } else {
            BakerySwapCall(0, amountGet, bdata);
            IBakerySwapPair(pairAddress).swap(
                0,
                amountGet,
                address(this)
    ...
    function BakerySwapCall(
        ...
        (uint112 reserve0, uint112 reserve1, ) = IBakerySwapPair(
    ...
    function calculateAmount(
        ...
        (uint112 reserve0, uint112 reserve1, ) = IBakerySwapPair(pairAddress)
            .getReserves();
        if (tokenNum == 0 && isGet) {
